...
/// Removes debug intrinsics like llvm.dbg.declare and llvm.dbg.value.
 ...
 public:
  ...
  void run(Module *M) {
    ...
    if (RemoveNamedInfo) {
      ...
      if (Node) M->eraseNamedMetadata(Node);
    }
  }
};
...
class DIUpdater : public InstVisitor<DIUpdater> {
 ...
 public:
  DIUpdater(Module &M, StringRef Filename = StringRef(),
            StringRef Directory = StringRef(), Module *DisplayM = nullptr,
  ...
  void visitModule(UNUSED_ATTRIBUTE Module &M) {
    if (Finder.compile_unit_count() > 1) {
      report_fatal_error(
          "DebugIR pass supports only a single compile unit per Module.");
    }
  ...
  void visitFunction(Function &F) {
    ...
    int FuncFlags = llvm::DINode::FlagPrototyped;

    PL_ASSERT(CUNode != nullptr && FileNode != nullptr);

    DISubprogram *Sub = Builder.createFunction(
        CUNode, F.getName(), MangledName, FileNode, Line, Sig, Local,
        IsDefinition, ScopeLine, FuncFlags, IsOptimized, &F);
    // PL_ASSERT(Sub->isSubprogram());
 ...
 private:
  ...
  /// if it is not.
  DIType *getType(const Type *type) {
    auto iter = TypeDescriptors.find(type);
    if (iter != TypeDescriptors.end()) {
      return iter->second;
    }
    return nullptr;
  }
  ...
  DIType *getOrCreateType(Type *type) {
    // Check the cache
    ...
    // Check if void, if so return nullptr for type
    if (type->isVoidTy()) {
      return nullptr;
    }
    ...
    if (type->isStructTy()) {
      // Forward declare the struct type to properly handle recursive members
      auto di_fwd_decl =
          llvm::TempDIType(Builder.createReplaceableCompositeType(
              llvm::dwarf::DW_TAG_structure_type, type->getStructName(), CUNode,
              FileNode, 0, llvm::dwarf::DW_LANG_C,
              Layout.getTypeSizeInBits(type), Layout.getABITypeAlignment(type),
              0, type->getStructName()));

      // Add forward declaration to type cache
      TypeDescriptors[type] = di_fwd_decl.get();

      // Get the elements of the struct
      SmallVector<Metadata *, 8> Elements;
      for (unsigned i = 0; i < type->getStructNumElements(); ++i) {
        Elements.push_back(getOrCreateType(type->getStructElementType(i)));
      }

      // Build the real struct
      di_type = Builder.createStructType(
          CUNode, type->getStructName(), FileNode, 0,
          Layout.getTypeSizeInBits(type), Layout.getABITypeAlignment(type), 0,
          nullptr,  // DerivedFrom
          Builder.getOrCreateArray(makeArrayRef(Elements)),
          llvm::dwarf::DW_LANG_C, nullptr, type->getStructName());

      // Replace the old type with the new one we just created
      Builder.replaceTemporary(std::move(di_fwd_decl), di_type);

      // Update the cache and return
      TypeDescriptors[type] = di_type;
      return di_type;
    }

    // Check pointer type
    if (type->isPointerTy()) {
      Type *pointee_type = type->getPointerElementType();
      di_type = getType(pointee_type);
      if (di_type == nullptr) {
        di_type = Builder.createPointerType(
            getOrCreateType(pointee_type),
            Layout.getPointerTypeSizeInBits(type),
            Layout.getPrefTypeAlignment(type), getTypeName(type));
      }
    ...
    if (type->isArrayTy()) {
      ...
      Subrange.push_back(
          Builder.getOrCreateSubrange(0, type->getArrayNumElements() - 1));

      di_type = Builder.createArrayType(
          Layout.getTypeSizeInBits(type), Layout.getPrefTypeAlignment(type),
          getOrCreateType(type->getArrayElementType()),
          Builder.getOrCreateArray(makeArrayRef(Subrange)));
    ...
    di_type = Builder.createBasicType(
        getTypeName(type), type->getPrimitiveSizeInBits(), 0, encoding);
  ...
  DISubroutineType *createFunctionSignature(const Function *func) {
    // All the parameters
    SmallVector<Metadata *, 8> params;

    // Put the function return type at tht 0-th index
    params.push_back(getOrCreateType(func->getReturnType()));

    const Function::ArgumentListType &Args(func->getArgumentList());
    for (Function::ArgumentListType::const_iterator i = Args.begin(),
                                                    e = Args.end();
         i != e; ++i) {
      params.push_back(getOrCreateType(i->getType()));
    }

    DITypeRefArray param_array =
        Builder.getOrCreateTypeArray(makeArrayRef(params));
    return Builder.createSubroutineType(FileNode, param_array);
  }
...
bool getSourceInfoFromModule(const Module &M, std::string &Directory,
  ...
  if (PathStr.length() == 0 || PathStr == "<stdin>") return false;
...
// Sets Directory/Filename from debug information in M and returns true, or
...
bool getSourceInfoFromDI(const Module &M, std::string &Directory,
  ...
  if (CUNode == nullptr || CUNode->getNumOperands() == 0) {
    return false;
  }
...
void DebugIR::createDebugInfo(Module &M, std::unique_ptr<Module> &DisplayM) {
  ...
  if (WriteSourceToDisk && (HideDebugIntrinsics || HideDebugMetadata)) {
    ...
    if (HideDebugIntrinsics) DebugIntrinsicsRemover::process(*DisplayM);

    if (HideDebugMetadata) DebugMetadataRemover::process(*DisplayM);
  }
